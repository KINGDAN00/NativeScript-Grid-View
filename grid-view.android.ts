...
interface GridViewAdapter<T> extends androidx.recyclerview.widget.RecyclerView.Adapter<T> {
    // tslint:disable-next-line:no-misused-new
    new(owner: WeakRef<GridView>): GridViewAdapter<T>;
}

let GridViewAdapter: GridViewAdapter<any>;
...
function initGridViewAdapter() {
    ...
    class GridViewAdapterImpl extends androidx.recyclerview.widget.RecyclerView.Adapter<GridViewCellHolder> {
        constructor(private owner: WeakRef<GridView>) {
            super();
        ...
        public onCreateViewHolder(parent: android.view.ViewGroup, viewType: number): GridViewCellHolder {
            const owner = this.owner.get();
